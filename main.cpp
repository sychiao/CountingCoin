...
int main()
{
	...
	int bu[] = { 1,1,1,1,1,
				...
				1,2,5,2,1,
				1,1,1,1,1 };
	int *bur = Convolution(img.pixel, img.w, img.h, bu, 5, 5);
	...
	int *gx = Convolution(img.pixel, img.w, img.h, a, 3, 3);
	int *gy = Convolution(img.pixel, img.w, img.h, b, 3, 3);
	...
	for (int i = 0; i < img.w; i++)
	{
		for (int j = 0; j < img.h; j++)
		{
			...
			if (value > 75)
				img.pixel[i + j*img.h] = 255;
			else
				img.pixel[i + j*img.h] = 0;
		}
	}
